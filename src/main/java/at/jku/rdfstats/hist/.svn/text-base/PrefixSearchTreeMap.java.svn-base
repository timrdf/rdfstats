...
public class PrefixSearchTreeMap<K, V>
    extends AbstractMapCopy<K,V>
    implements SortedMap<K,V>, Cloneable, java.io.Serializable {
	
	    /**
	     * The Comparator used to maintain order in this PrefixSearchTreeMap, or
	     * null if this PrefixSearchTreeMap uses its elements natural ordering.
	     *
	     * @serial
	     */
	    ...
	    /**
	     * Removes the mapping for this key from this PrefixSearchTreeMap if present.
	     *
	    ...
	    /**
	     * Removes all mappings from this PrefixSearchTreeMap.
	     */
	    public void clear() {
	        modCount++;
	        size = 0;
	    ...
	    /**
	     * Returns a shallow copy of this <tt>PrefixSearchTreeMap</tt> instance. (The keys and
	     ...
	     * this <tt>PrefixSearchTreeMap</tt> instance, so changes to this map are reflected in
	     ...
	     *
	     * @return a set view of the keys contained in this PrefixSearchTreeMap.
	     */
	    public Set<K> keySet() {
	        if (keySet == null) {
	     ...
	     * mapping from the PrefixSearchTreeMap, through the <tt>Iterator.remove</tt>,
	        ...
	        private class EntrySetView extends AbstractSet<Map.Entry<K,V>> {
	            ...
	            public int size() {
	                if (size == -1 || sizeModCount != PrefixSearchTreeMap.this.modCount) {
	                    size = 0;  sizeModCount = PrefixSearchTreeMap.this.modCount;
	                    Iterator i = iterator();
	                    while (i.hasNext()) {
	                        size++;
	                        i.next();
	    ...
	    /**
	     * Compares two keys using the correct comparison method for this PrefixSearchTreeMap.
	     */
	    ...
	    /**
	     * Returns the first Entry in the PrefixSearchTreeMap (according to the PrefixSearchTreeMap's
	     * key-sort function).  Returns null if the PrefixSearchTreeMap is empty.
	     */
	    ...
	    /**
	     * Returns the last Entry in the PrefixSearchTreeMap (according to the PrefixSearchTreeMap's
	     * key-sort function).  Returns null if the PrefixSearchTreeMap is empty.
	     */
	    ...
	    /**
	     * Save the state of the <tt>PrefixSearchTreeMap</tt> instance to a stream (i.e.,
	     * serialize it).
	     *
	     * @serialData The <i>size</i> of the PrefixSearchTreeMap (the number of key-value
	     *             mappings) is emitted (int), followed by the key (Object)
	     *             and value (Object) for each key-value mapping represented
	     *             by the PrefixSearchTreeMap. The key-value mappings are emitted in
	     *             key-order (as determined by the PrefixSearchTreeMap's Comparator,
	     *             or by the keys' natural ordering if the PrefixSearchTreeMap has no
	     *             Comparator).
	     */
	    ...
	    /**
	     * Reconstitute the <tt>PrefixSearchTreeMap</tt> instance from a stream (i.e.,
	     * deserialize it).
	     */
	     ...
	     *
	     * It is assumed that the comparator of the PrefixSearchTreeMap is already set prior
	     ...
	     * It is assumed that the comparator and size fields of the PrefixSearchTreeMap are
	    ...
	    /**
	     * find closest prefix (only works for String keys)
	     * e.g. finds "Dani" for key "Daniel" but not "Daniel" for key "Dani"
	     * 
	     * This algorithm is based on the assumption that the keys stored by this
	     * prefix search tree are incomplete (trimmed bin labels of a {@link OrderedStringHistogram}).
	     * It helps to find the closest matching bin label for a given value. 
	     * 
	     * @param key
	     * @return
	     */
		...
		private Entry<K, V> getClosestEntry(String key) {
			...
			// starting with root as current entry p
			Entry<K,V> p = root;
			Stack<Entry<K,V>> stack = new Stack<Entry<K,V>>();
			
			// go down into the tree and fill the stack
	        while (p != null) {
				stack.push(p);
	            int cmp = compare(k, p.key);
	            ...
	            // descend left or right until p == null
	            } else if (cmp < 0)
	            	p = p.left;
	            else
	            	p = p.right;
	        ...
	        // now, look for the latest entry in the stack starting with key
			while (!stack.isEmpty()) {
				p = stack.pop();
				if (key.startsWith((String) p.key))
					return p;
			}
